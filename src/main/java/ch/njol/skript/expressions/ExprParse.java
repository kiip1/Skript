	...
	protected Object[] get(Event event) {
		...
		try {
			...
			if (classInfo != null) {
				...
				if (matchResult != null) {
					List<Expression<?>> exprs = matchResult.expressions();

					assert patternExpressionPlurals.length == exprs.size();
					...
					for (int i = 0; i < exprs.size(); i++) {
						if (exprs.get(i) != null) {
							...
							values[valueIndex] = patternExpressionPlurals[i] ? exprs.get(i).getArray(null) : exprs.get(i).getSingle(null);
